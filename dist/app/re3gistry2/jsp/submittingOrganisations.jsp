...
<html lang="${localization.getString("property.localeid")}" role="document">
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col">
                    <h1 class="page-heading">${localization.getString("submittingorganisations.label.title")}</h1>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-sm-3">
                    <a href=".<%=WebConstants.PAGE_URINAME_SUBMITTINGORGANISATIONS%>" class="btn btn-light float-right width100" role="button" aria-pressed="true">
                    ...
                    <tbody>
                        <%                            for (RegAction tmp : regActions) {
                                if (regActions.size() == 1) {
                                    if ((tmp.getRegStatus().getLocalid().equals(BaseConstants.KEY_STATUS_LOCALID_DRAFT)
                                            ...
                                            && tmp.getRegUser().getUuid().equals(regUser.getUuid())) {
                                        showDiscard = true;
                                    }
                                }
                        %>
                        <tr>   
                            <td>
                                ...
                                <%if (regAction != null && tmp.getRegUser().getUuid().equals(regUser.getUuid()) && (tmp.getRegStatus().getLocalid().equals(BaseConstants.KEY_STATUS_LOCALID_DRAFT) && tmp.getChangeRequest() == null)) {%>


                                <input type="hidden" name="<%=BaseConstants.KEY_FORM_FIELD_NAME_ACTIONUUID%>" id="<%=BaseConstants.KEY_FORM_FIELD_NAME_ACTIONUUID%>" value="<%=tmp.getUuid()%>"/>
                                <input type="hidden" name="<%=BaseConstants.KEY_FORM_FIELD_NAME_UPDATELABELACTION%>" value="true" />                            

                                <label class="text_label"><%=tmp.getLabel()%></label>
                                <div class="edit float-right" title="<%=systemLocalization.getString("label.updatelabel")%>"><i class="fas fa-edit text-success"></i></div>
                                <input required class="input_edit" id="input_edit" type="text" name="<%=BaseConstants.KEY_FORM_FIELD_NAME_LABEL%>" value="<%=tmp.getLabel()%>" style="width:100%; display: none"  maxlength="<%= configuration.getProperties().getProperty("application.input.maxlength")%>" />


                                <%} else if (regAction != null) {%>
                                <%=tmp.getLabel()%>
                                <%} else {%>
 ...
 /* -- Change request  -- */
                ...
                <tbody>
                    <%
                        for (RegItemproposed tmp : regItemproposeds) {
                            ...
                            try {
                                regLocalizationproposeds = regLocalizationProposedManager.getAll(regFieldTitle, tmp, regAction);
                                if (regLocalizationproposeds.isEmpty()) {
                                    regLocalizationproposeds = regLocalizationProposedManager.getAll(regFieldTitle, tmp, masterLanguage);
                                }
                            } catch (NoResultException e) {
                            }
                    ...
                    <tr>
                        ...
                        <td><%
                            if ((tmp.getRegStatus().getLocalid().equals(BaseConstants.KEY_STATUS_LOCALID_DRAFT)
                                    ...
                                    && showDiscard) {
                            %>
                            <a data-toggle="confirmation" data-title="${localization.getString("discard.proposal.confirm")}" data-placement="left" data-singleton="true" 
                               class="btn btn-sm btn-danger btn-approve-action  btn-reg-action" 
                               href=".<%=WebConstants.PAGE_URINAME_DISCARDPROPOSAL%>?<%=BaseConstants.KEY_REQUEST_ITEMUUID%>=<%=tmp.getUuid()%>&<%=BaseConstants.KEY_REQUEST_ACTION_UUID%>=<%=tmp.getRegAction().getUuid()%>"
                               ><i class="far fa-trash-alt"></i> ${localization.getString("label.discardcurrentchanges")}</a>
                            <%
                                }
                            %>
                        </td>
                ...
                <tbody>
                    <%
                        for (RegItemhistory tmp : regItemhistorys) {
                            ...
                            try {
                                regLocalizationhistorys = regLocalizationhistoryManager.getAll(regFieldTitle, tmp, regAction);
                                if (regLocalizationhistorys.isEmpty()) {
                                    regLocalizationhistorys = regLocalizationhistoryManager.getAll(regFieldTitle, tmp, masterLanguage);
                                }
                            } catch (Exception e) {
                            }
        ...
        <script src="./res/js/easy-editable-text.js"></script>
