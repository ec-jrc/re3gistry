...
<%    // ## Get the error related to the Procedure passed by ID (pid parameter) ##
    ...
    if (detailedUser != null && pid!=null && pid.length()>0) {
        ...
        if (u != null && u.getActive()) {
            ...
            Procedure procedure = ProcedureMgr.getProcedureByID(pid);
            ...
            if(procedure!=null && (procedure.getStartedby().getOrganization().getUuid().equals(o.getUuid()) || u.getUserlevel().equals(Constants.USER_LEVEL_ADMIN)) && ((procedure.getStatus().equals(Constants.PROCEDURE_STATUS_ERROR) || procedure.getStatus().equals(Constants.PROCEDURE_STATUS_SUCCESS)))){
                // Getting the report message    
                
                // CHecking if the user has the right to show the current report
                if(u.getUserlevel()==Constants.USER_LEVEL_ADMIN || (u.getUserlevel()!=Constants.USER_LEVEL_ADMIN && u.getOrganization().getUuid().equals(procedure.getOrganization().getUuid()))){
                    outhtml = procedure.getMessage();
                }else{
