...
public class RegisterManager extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        String[] actionPublishProposals = {BaseConstants.KEY_USER_ACTION_PUBLISHPROPOSAL};
        boolean permissionPublishProposals = UserHelper.checkGenericAction(actionPublishProposals, currentUserGroupsMap, regItemRegGroupRegRoleMappingManager);

        if (permissionPublishProposals) {
            ...
            if (formRegActionUuid != null && formRegActionUuid.length() > 0 && formSubmitAction != null && formSubmitAction.length() > 0) {
                ...
                regActionHandler.registerManagerAction(formRegActionUuid, regUser);
            ...
            List<RegItemRegGroupRegRoleMapping> tmps = regItemRegGroupRegRoleMappingManager.getAll(regRoleRegisterManager);

            // Getting the action for which the current user is RM
