...
public class MigrationManager {
    ...
    private void createRegGroupLocalization(Reference reference, int fieldIndex, RegItem registerItem, RegField regField, boolean commit) throws Exception {
...
//        languagecodeList.forEach((languagecode) -> {
        ...
        //get localization of selected reference
        Query queryLocalizationReference = entityManagerRe3gistry2Migration.createQuery(ConstantsMigration.LOCALIZATION_BY_REFERENCE);
//            Query queryLocalizationReference = entityManagerRe3gistry2Migration.createQuery(ConstantsMigration.LOCALIZATION_BY_REFERENCE_AND_LANGUAGE);
        queryLocalizationReference.setParameter("reference", reference);
        List<Localization> referenceLocalizationList;
        try {
//                queryLocalizationReference.setParameter("language", languagecode);
            referenceLocalizationList = queryLocalizationReference.getResultList();
        } catch (Exception ex) {
            logger.error("Error in  getting the result list for " + queryLocalizationReference + " " + ex.getMessage());
            throw new Exception("Error in  getting the localization for " + queryLocalizationReference + " " + ex.getMessage());
        }
        for (Localization referenceLocalization : referenceLocalizationList) {
            ...
            } else if (reference.getEmail() != null) {
                regLocalization.setHref("mailto:" + reference.getEmail());
            } else {
